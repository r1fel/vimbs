...
import {
  ...
  WinterSubcategories,
  SummerSubcategories,
} from '../../src/controllers/itemControllers';
...
const checkResponseToBeCorrectlyProcessedItemForClient = ({
  ...
  const correctlyProcessedItemForClient = {
    ...
    categories: {
      AdultClothing: {
        name: 'Mode',
        subcategories: validBody.categories.AdultClothing?.subcategories ?? [],
      },
      ChildAndBaby: {
        name: 'Kind und Baby',
        subcategories: validBody.categories.ChildAndBaby?.subcategories ?? [],
      },
      HouseAndGarden: {
        name: 'Haus und Garten',
        subcategories: validBody.categories.HouseAndGarden?.subcategories ?? [],
      },
      MediaAndGames: {
        name: 'Medien und Spiele',
        subcategories: validBody.categories.MediaAndGames?.subcategories ?? [],
      },
      Other: {
        name: 'Sonstiges',
      ...
      Technology: {
        ...
        subcategories: validBody.categories.Technology?.subcategories ?? [],
      },
    },
    owner: {
      _id: expect.any(String), // _id should be a mongo.Types.ObjectId, represented as a String
...
describe('item Routes', () => {
  ...
  describe(`GET ${itemSuggestRoute} (suggestItems)`, () => {
    describe('when avialble items exist', () => {
      ...
      it(
        'should respond successful with a statusCode200 and return an array of ' +
          String(expectedNumberOfItems) +
          ' items',
        async () => {
          const connectSidValue = await loginBodo4();
          ...
          expect(suggestItemsResponse.body.length).toEqual(
  ...
  describe(`helper functions of suggestItems`, () => {
    ...
    describe('getRandomItems for valid input', () => {
      it('should return an array of random selected items', async () => {
        ...
        let itemsArray: ItemInDBPopulated[] = Array.isArray(items)
          ? (items.filter((item) => item !== null) as ItemInDBPopulated[])
          : ([items].filter((item) => item !== null) as ItemInDBPopulated[]);
    ...
    describe('getSearchHistoryItems for valid input', () => {
      it('should return items, which names match the search token and their creation dates are newer than the search date', async () => {
        const connectSidValue = await loginBodo4();
        const items = await getPopulatItems();
        const itemsArray: ItemInDBPopulated[] = Array.isArray(items)
          ? items
          : items !== null
          ? [items]
          : [];
        ...
        if (globalUserBodo.searchHistory.length > 0) {
          globalUserBodo.searchHistory = [] as unknown as [
            { searchToken: string; date: Date },
          ];
        }

        globalUserBodo?.searchHistory.push({
          searchToken: expected_history_items,
          date: getRandomDateIn1950(),
        });

        const searchHistoryItems = await getSearchHistoryItems(
          itemsArray,
          globalUserBodo,
        );
    ...
    describe('getSearchHistoryItems for empty input', () => {
      it('should return empty array', async () => {
        ...
        const emptySearchHistoryItems = await getSearchHistoryItems(
          [],
          globalUserBodo,
        );
    ...
    describe('filterItemsBySeason for valid input', () => {
      it('should return an array of objects, filtering for items that are out of season', async () => {
        ...
        itemsArray[0].categories = {
          HouseAndGarden: {
            name: 'HouseAndGarden',
            subcategories: [SummerSubcategories[1]],
          },
        };
        itemsArray[1].categories = {
          SportAndCamping: {
            name: 'SportAndCamping',
            subcategories: [WinterSubcategories[0]],
          },
        };
        ...
        if (currentMonth < 3 || currentMonth > 10) {
          for (const token of SummerSubcategories) {
            const summerItems = Array.isArray(filterItemsBySeasonResponse)
              ? filterItemsBySeasonResponse.filter((item: ItemInDBPopulated) =>
                  item.categories?.HouseAndGarden?.subcategories?.includes(
                    token,
                  ),
                )
              : [];
          ...
          for (const token of WinterSubcategories) {
            const winterItems = Array.isArray(filterItemsBySeasonResponse)
              ? filterItemsBySeasonResponse.filter((item: ItemInDBPopulated) =>
                  item.categories?.HouseAndGarden?.subcategories?.includes(
                    token,
                  ),
    ...
    describe('getItemsBasedonCatagory for valid input', () => {
      it('should return array of items, which only belong to the subcatagories of Sport and Garden', async () => {
        ...
        globalUserBodo?.searchHistory.push({
          searchToken: expected_catagory_items,
          date: getRandomDateIn1950(),
        });
        ...
        globalUserBodo?.getHistory.push(
          new mongoose.Types.ObjectId(expected_catagory_item_id),
        );
        const objectids = [
          new mongoose.Types.ObjectId(expected_catagory_item_id),
        ];
        ...
        const itemsBasedonCatagory = await getItemsBasedOnCatagories(
          itemsArray,
          globalUserBodo,
          1,
        );
        ...
        const arrayItemsBased: ItemInDBPopulated[] = Array.isArray(
          itemsBasedonCatagory,
        )
          ? itemsBasedonCatagory
          : itemsBasedonCatagory !== null
          ? [itemsBasedonCatagory]
          : [];
      ...
      describe('getItemsBasedonCatagory for empty input', () => {
        it('should return an empty array', async () => {
          ...
          const itemsArray: ItemInDBPopulated[] = Array.isArray(items)
            ...
            ? [items]
            : [];
          const emptyInputResponse = await getItemsBasedOnCatagories(
            [],
            globalUserBodo,
            2,
          );
