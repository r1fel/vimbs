...
module.exports.createBorrowingrequest = async (req, res) => {
  ...
  borrowingrequest.textlog.push({
    messageText: message,
    messageWriter: 'b',
    messageTimestamp: reqTimestamp,
  });
  borrowingrequest.bookLocationLog.push({
    newBookLocation: status,
    bookLocationChanger: 'b',
...
module.exports.deleteBorrowingrequest = async (req, res) => {
  ...
  await Book.findByIdAndUpdate(id, {
    $pull: {borrowingrequests: borrowingrequestId},
  });
...
module.exports.handlePostBorrowingrequest = async (req, res) => {
  ...
  const pushMessage = () => {
    if (message) {
      borrowingrequest.textlog.push({
        messageText: message,
        messageWriter: book.owner.equals(requserid) ? 'l' : 'b',
        messageTimestamp: reqTimestamp,
      });
    }
  };
  const pushBookLocationChange = () => {
    borrowingrequest.bookLocationLog.push({
      newBookLocation: status,
      bookLocationChanger: book.owner.equals(requserid) ? 'l' : 'b',
      entryTimestamp: reqTimestamp,
    });
  };

  //TODO ER check logic thoroghly
  ...
  //? ER FR: simplify - can we do this better?
  ...
  //decline a borrowingRequest by b or l
  if (borrowingrequest.bookLocation === 'home' && status === 'declined') {
    ...
    await borrowingrequest.save();
    //change status from home to transferLtoB
  } else if (
    borrowingrequest.bookLocation === 'home' &&
    status === 'transferLtoB' &&
    book.owner.equals(requserid)
  ) {
    borrowingrequest.bookLocation = status;
    setDueDate();
    pushMessage();
    pushBookLocationChange();
    await borrowingrequest.save();
    //borrower recieved the book
  } else if (
    borrowingrequest.bookLocation === 'transferLtoB' &&
    status === 'atB' &&
    borrowingrequest.borrower.equals(requserid)
  ) {
    borrowingrequest.bookLocation = status;
    pushMessage();
    pushBookLocationChange();
    await borrowingrequest.save();
    //borrower initiates transferBtoL
  } else if (
    borrowingrequest.bookLocation === 'atB' &&
    status === 'transferBtoL' &&
    borrowingrequest.borrower.equals(requserid)
  ) {
    borrowingrequest.bookLocation = status;
    ...
    borrowingrequest.bookLocation = status;
    const reqTimestampPlus3days = new Date(
      reqTimestamp.setDate(reqTimestamp.getDate() + 3),
    );
    ...
    borrowingrequest.bookLocation === 'transferBtoL' &&
    status === 'backHome' &&
    book.owner.equals(requserid)
  ) {
    borrowingrequest.bookLocation = status;
  ...
  const updatedBook = {
    ...
    image: response.image,
    blurb: response.blurb,
    available: ['backHome', 'declined'].includes(
      response.borrowingrequests[response.borrowingrequests.length - 1]
        .bookLocation,
    )
      ? true
      : false,
    dueDate: ['backHome', 'declined'].includes(
      response.borrowingrequests[response.borrowingrequests.length - 1]
        .bookLocation,
    )
      ? ''
    ...
    borrowingrequests: book.owner.equals(requserid)
